...
public interface ScheduleRepository extends JpaRepository<ScheduleVO, Integer> {
	...
	@Query(nativeQuery = true, value ="select m.title, s.start, m.addr1, m.firstimage,s.cost,s.tour,s.arCode, s.scName,s.day "
			+ "from mytour m join schedule s on m.title=s.tour  and m.plNum=s.plNum where m.title=:title and m.plNum=:plNum")
